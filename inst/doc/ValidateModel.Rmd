...
#### 2.1. Check if commodity output from state Make and Use are almost equal (<= 0.01).
```{r check2.1}
failures <- c()
for (state in states[states != "Overseas"]) {
...
#### 2.2. Check if commodity output from state Use and TwoRegion Use are almost equal (<= 0.01).
```{r check2}
failures <- c()
for (state in states[states != "Overseas"]) {
  # Calculate Commodity Output (q) from Make and Use
  q <- TwoRegionCommodityOutput_ls[[state]]
  q_2r_use <- rowSums(TwoRegionDomesticUse_ls[[state]])
  # Calculate relative differences in q_make and q_use
...
validateTwoRegionLagainstOutput <- function(state, year, ioschema, iolevel) {
  ...
  validation$rel_diff <- validation$`L*y-output`/c(SoI_Commodity_Output,
                                                   RoUS_Commodity_Output,
                                                   SoI_Industry_Output,
                                                   RoUS_Industry_Output)
  validation$Ly <- as.numeric(L %*% y)
  validation$output <- c(SoI_Commodity_Output$Output,
