...
for (state in states[states!="Overseas"]) {
  ...
  failures_state <- formatValidationResult(rel_diff, abs_diff = TRUE, tolerance = 1E-3)[["Failure"]]
  if (nrow(failures_state)>0) {
    failures_state$State <- state
  }
  colnames(failures_state) <- c("Industry", "")
...
#### 2. Check if commodity output from state Make and Use are almost equal (<= 0.001).
```{r check1}
failures <- c()
for (state in states[states!="Overseas"]) {
...
#### 5. Sum of each industry's output across all states must almost equal(<= 0.001) industry output in US Make Table.
```{r check4}
...
#### 6. Sum of each commodity's output across all states must almost equal(<= 0.001) commodity output in US Make Table.
```{r check5}
...
#### 7. Sum of each commodity's output across all states must almost equal (<= 1E^7, or $10 million by commodity) commodity output in US Use Table minus International Imports (commodity specific).
Note: even if the threshold is met, track the difference for each commodity. Save result as a type of quality control check.

```{r check6}
df0 <- as.data.frame(rowSums(US_Summary_Use) - US_Summary_DomesticUse$F051)
df1 <- Reduce("+", State_Summary_CommodityOutput_ls)
rownames(df1) <- gsub(".*\\.", "", rownames(df1))
df1 <- df1[rownames(df0), ]
...
#### 13. Check row sum of SoI2SoI <= state commodity supply. Row sum of RoUS2RoUS <= national commodity supply.
```{r check12}
failures <- data.frame()
for (state in states[states!="Overseas"]) {
...
#### 16. Total state commodity supply == state demand by intermediate consumption, plus final demand (except imports) + Interregional Exports + Export Residual. Difference must be <= 0.001.
```{r check15}
failures <- data.frame()
for (state in states[states!="Overseas"]) {
  # Prepare domestic 2-region Use tables
