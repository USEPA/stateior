...
buildStateUseModel <- function(year) {
  ...
  # Because in US_Import matrix, row sum of non-import columns = US_Import$F050
  # (with a +/- X million $ difference), we assume this equation still holds at
  # state level. Therefore, a State_Import$F050 is needed. Although it's unknown,
  # we can calculate it by subtracting state ITA from StateUse$F050, reusing the
  # method for calculating ITA.
  ...
  for (state in states) {
    ...
    State_Import_m <- (State_Use$F050 - State_Use$F051) * commodity_import_ratio
...
buildTwoRegionUseModel <- function(state, year, ioschema, iolevel,
  ...
  # 9.1 - Allocate negative InterregionalExports across columns in SoI2SoI Use and RoUS2RoUS Use
  logging::loginfo("Allocating negative interregional exports in SoI2SoI Use and RoUS2RoUS Use...")
  for (i in rows_allocation) {
    # SoI2SoI
    if (SoI2SoI_Use[i, "InterregionalExports"] < 0) {
  ...
  if (domestic) {
    q_SoI_use <- rowSums(SoI2SoI_Use[, c(industries, FD_cols, ITA_col, "ExportResidual")]) + rowSums(SoI2RoUS_Use[, c(industries, FD_cols)])
  } else {
  ...
  if (domestic) {
    q_RoUS_use <- rowSums(RoUS2RoUS_Use[, c(industries, FD_cols, ITA_col, "ExportResidual")]) + rowSums(RoUS2SoI_Use[, c(industries, FD_cols)])
  } else {
